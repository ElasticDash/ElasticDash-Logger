...
<div align="center">
  ...
  <br/>
  <span>Langfuse 使用 <a href="https://github.com/orgs/elasticdash/discussions"><strong>GitHub Discussions</strong></a> 作为支持和功能请求的平台。</span>
  <br/>
...
[![Langfuse 概览视频](https://github.com/user-attachments/assets/3926b288-ff61-4b95-8aa1-45d041c70866)](https://elasticdash.com/watch-demo)

## ✨ 核心特性

![Langfuse 概览](https://elasticdash.com/images/docs/github-readme/github-feature-overview.png)

- [LLM 应用可观察性](https://elasticdash.com/docs/tracing)：为你的应用插入仪表代码，并开始将追踪数据传送到 Langfuse，从而追踪 LLM 调用及应用中其他相关逻辑（如检索、嵌入或代理操作）。检查并调试复杂日志及用户会话。试试互动的 [演示](https://elasticdash.com/docs/demo) 看看效果。
- [提示管理](https://elasticdash.com/docs/prompt-management/get-started) 帮助你集中管理、版本控制并协作迭代提示。得益于服务器和客户端的高效缓存，你可以在不增加延迟的情况下反复迭代提示。

- [评估](https://elasticdash.com/docs/evaluation/overview) 是 LLM 应用开发流程的关键组成部分，Langfuse 能够满足你的多样需求。它支持 LLM 作为"裁判"、用户反馈收集、手动标注以及通过 API/SDK 实现自定义评估流程。
...
- [综合 API](https://elasticdash.com/docs/api)：Langfuse 常用于驱动定制化的 LLMOps 工作流程，同时利用 Langfuse 提供的构建模块和 API。我们提供 OpenAPI 规格、Postman 集合以及针对 Python 和 JS/TS 的类型化 SDK。

## 📦 部署 Langfuse

![Langfuse 部署选项](https://elasticdash.com/images/docs/github-readme/github-deployment-options.png)
...
- [本地（docker compose）](https://elasticdash.com/self-hosting/local)：使用 Docker Compose 在你的机器上于 5 分钟内运行 Langfuse。
...
- [Kubernetes（Helm）](https://elasticdash.com/self-hosting/kubernetes-helm)：使用 Helm 在 Kubernetes 集群上部署 Langfuse。这是推荐的生产环境部署方式。

- [虚拟机](https://elasticdash.com/self-hosting/docker-compose)：使用 Docker Compose 在单台虚拟机上部署 Langfuse。
...
![Langfuse 集成](https://elasticdash.com/images/docs/github-readme/github-integrations.png)
...
1. [创建 Langfuse 账户](https://cloud.elasticdash.com/auth/sign-up) 或 [自托管](https://elasticdash.com/self-hosting)
2. 创建一个新项目
3. 在项目设置中创建新的 API 凭证

### 2️⃣ 记录你的第一个 LLM 调用

使用 [<code>@observe()</code> 装饰器](https://elasticdash.com/docs/sdk/python/decorators) 可轻松跟踪任何 Python LLM 应用。在本快速入门中，我们还使用了 Langfuse 的 [OpenAI 集成](https://elasticdash.com/integrations/model-providers/openai-py) 来自动捕获所有模型参数。
...
_[Langfuse 中的公共示例追踪](https://cloud.elasticdash.com/project/cloramnkj0002jz088vzn1ja4/traces/2cec01e3-3dc2-472f-afcf-3b968cf0c1f4?timestamp=2025-02-10T14%3A27%3A30.275Z&observation=cb5ff844-07ef-41e6-b8e2-6c64344bc13b)_

> [!提示]
>
> [了解更多](https://elasticdash.com/docs/tracing) 关于 Langfuse 中的追踪，或试试 [互动演示](https://elasticdash.com/docs/demo)。
...
- [Langfuse 常见问题](https://elasticdash.com/faq) 解答了最常见的问题。
- 使用 "Ask AI" 立即获取问题答案。
...
以下是使用 Langfuse 的顶级开源 Python 项目，按星标数排名（[来源](https://github.com/elasticdash/elasticdash-docs/blob/main/components-mdx/dependents)）：

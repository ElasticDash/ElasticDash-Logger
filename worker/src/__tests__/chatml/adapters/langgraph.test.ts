...
import { langgraphAdapter, normalizeInput } from "@elasticdash/shared";
...
describe("LangGraph Adapter", () => {
  it("should detect LangGraph via various metadata patterns", () => {
    ...
    expect(
      langgraphAdapter.detect({
        metadata: { scope: { name: "elasticdash-sdk" } },
      }),
    ).toBe(false);
  });
  ...
  it("should skip null entries in tool_calls arrays", () => {
    ...
    expect(toolCalls?.[0].arguments).toBe('{"topic":"ElasticDash"}');
  });

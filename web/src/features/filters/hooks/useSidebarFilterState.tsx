...
export function useSidebarFilterState(
  ...
  // init default env filters on first load to deselect envs prefixed with "elasticdash-"
  useEffect(() => {
    ...
    const elasticdashEnvironments = environments.filter((env) =>
      env.startsWith("elasticdash-"),
    );

    // exclude elasticdash- environments if there are any
    if (elasticdashEnvironments.length > 0) {
      const defaultFilter: FilterState = [
        {
          column: "environment",
          type: "stringOptions",
          operator: "none of",
          value: elasticdashEnvironments,
        },
      ];

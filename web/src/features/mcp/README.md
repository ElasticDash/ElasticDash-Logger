# ElasticDash MCP Server

Model Context Protocol (MCP) server for ElasticDash, enabling AI assistants to interact with your ElasticDash prompts programmatically.

## Quick Start (Local Development)

### Prerequisites

- ElasticDash instance running locally
...
3. **Add to Claude Code**
   ...
   ```bash
   claude mcp add --transport http elasticdash http://localhost:3000/api/public/mcp \
...
ElasticDash supports **prompt composition** where prompts can reference other prompts via dependency tags like `@@@elasticdashPrompt:name=xxx|label=yyy@@@`. The MCP server provides two tools for fetching prompts with different resolution behaviors:
...
- **Example**:
  ```
  Input:  "You are helpful. @@@elasticdashPrompt:name=base-rules|label=production@@@"
...
- **Returns**: Original prompt content with `@@@elasticdashPrompt:...@@@` tags preserved
- **Example**:
  ```
  Input:  "You are helpful. @@@elasticdashPrompt:name=base-rules|label=production@@@"
  Output: "You are helpful. @@@elasticdashPrompt:name=base-rules|label=production@@@"
  ```
...
The ElasticDash MCP server uses a **stateless per-request architecture**:
...
All clients require BasicAuth authentication using your ElasticDash API keys.

### 1. Generate Basic Auth Token

Encode your ElasticDash API keys (Public Key:Secret Key) to base64:
...
### 2. Choose Your ElasticDash URL

**ElasticDash Cloud:**

- **EU Region:** `https://cloud.elasticdash.com`
- **US Region:** `https://us.elasticdash.com`
- **HIPAA:** `https://hipaa.elasticdash.com`
...
Register the ElasticDash MCP server:

```bash
# ElasticDash Cloud (EU)
claude mcp add --transport http elasticdash https://cloud.elasticdash.com/api/public/mcp \
    --header "Authorization: Basic {your-base64-token}"

# ElasticDash Cloud (US)
claude mcp add --transport http elasticdash https://us.elasticdash.com/api/public/mcp \
    --header "Authorization: Basic {your-base64-token}"

# Self-Hosted (HTTPS required)
claude mcp add --transport http elasticdash https://your-domain.com/api/public/mcp \
    --header "Authorization: Basic {your-base64-token}"
...
```json
{
  "mcp": {
    "servers": {
      "elasticdash": {
        "url": "https://cloud.elasticdash.com/api/public/mcp",
        "headers": {
...
Replace `https://cloud.elasticdash.com` with your ElasticDash URL (see [Choose Your ElasticDash URL](#2-choose-your-elasticdash-url)).

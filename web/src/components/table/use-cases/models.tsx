...
import { type Prisma } from "@elasticdash/shared/src/db";
import {
  useQueryParams,
  withDefault,
  NumberParam,
...
export default function ModelTable({ projectId }: { projectId: string }) {
  ...
  const columns: LangfuseColumnDef<ModelTableRow>[] = [
    ...
    },
    {
      ...
      size: 60,
      cell: ({ row }) => {
        const isLangfuse = row.original.maintainer === "ElasticDash";
        return (
          <div className="flex justify-center">
            <Tooltip>
              ...
              <TooltipContent>
                {isLangfuse ? "ElasticDash maintained" : "User maintained"}
              </TooltipContent>
            </Tooltip>
          </div>
        );

...
const ENVIRONMENT_NAME_REGEX_ERROR_MESSAGE =
  ...
  " and it must not start with 'elasticdash'";

const PublicEnvironmentName = z
  .string()
  .toLowerCase()
  .max(40, "Maximum length is 40 characters")
  .regex(/^(?!elasticdash)[a-z0-9-_]+$/, ENVIRONMENT_NAME_REGEX_ERROR_MESSAGE)
  .default("default");
 ...
 * @param isElasticDashInternal - Whether the events are being ingested by ElasticDash internally (e.g. traces created for prompt experiments).
 * @returns The ingestion event schema.
 */
export const createIngestionEventSchema = (isElasticDashInternal = false) => {
  return isElasticDashInternal

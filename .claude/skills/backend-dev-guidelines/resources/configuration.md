...
const region = env.NEXT_PUBLIC_ELASTICDASH_CLOUD_REGION;
```
...
const concurrency = env.ELASTICDASH_INGESTION_QUEUE_PROCESSING_CONCURRENCY;
const s3Bucket = env.ELASTICDASH_S3_EVENT_UPLOAD_BUCKET;
```
...
if (env.NEXT_PUBLIC_ELASTICDASH_CLOUD_REGION === "HIPAA") {
  // HIPAA compliance features
}

// Development/staging checks
if (env.NEXT_PUBLIC_ELASTICDASH_CLOUD_REGION === "DEV") {
...
NEXT_PUBLIC_ELASTICDASH_CLOUD_REGION=DEV

# Cloud US deployment
NEXT_PUBLIC_ELASTICDASH_CLOUD_REGION=US

# Self-hosted OSS deployment
# (variable not set)
```

### ELASTICDASH_EE_LICENSE_KEY
...
|-----------------------|--------------------|---------------------------------------------------------------------|
| **ElasticDash Cloud** | Not set            | Cloud features controlled by `NEXT_PUBLIC_ELASTICDASH_CLOUD_REGION` |
...
When `ELASTICDASH_EE_LICENSE_KEY` is set and valid:
...
if (env.ELASTICDASH_EE_LICENSE_KEY) {
  // Validate license
  const isValidLicense = await validateEELicense(env.ELASTICDASH_EE_LICENSE_KEY);
...
# ELASTICDASH_EE_LICENSE_KEY not set

# EE self-hosted
ELASTICDASH_EE_LICENSE_KEY=ee_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

# ElasticDash Cloud (uses region instead)
NEXT_PUBLIC_ELASTICDASH_CLOUD_REGION=US
# ELASTICDASH_EE_LICENSE_KEY not used
```
...
ELASTICDASH_LOG_PROPAGATED_HEADERS: z.string().optional().transform((s) =>
  s ? s.split(",").map((s) => s.toLowerCase().trim()) : []
),

// Parse project:rate pairs
ELASTICDASH_INGESTION_PROCESSING_SAMPLED_PROJECTS: z.string().optional().transform((val) => {

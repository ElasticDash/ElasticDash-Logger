...
import { paginationZod, singleFilter, orderBy } from "@elasticdash/shared";
import {
  getScoresUiTable,
  getScoresUiCount,
  upsertScore,
...
import { eventTypes, processEventBatch } from "@elasticdash/shared/src/server";
...
export async function createScoreWithValidation({
  ...
  if (!config) {
    throw new ElasticDashNotFoundError("Score config not found");
  }
